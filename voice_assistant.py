"""
Local voice assistant: whisper.cpp (STT) -> Qwen via Ollama (LLM) -> Kokoro (TTS)
Turn-based loop: press Enter, speak, hear the reply.

Prereqs (see setup instructions):
  - Ollama running with the model pulled:  ollama pull qwen3:8b
...
def chat(text, history):
    """Send the turn to Ollama. think=False keeps qwen3 out of reasoning mode
    so we don't speak its scratch work; the regex strip is a belt-and-suspenders."""
...
def main():
    history = [{
        ...
        "content": "You are a concise, friendly office concierge of Accenture Singapore. "
                   ...
                   "For Singapore generic queries, you may search the internet for more information. "
